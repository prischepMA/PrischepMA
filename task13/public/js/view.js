...
let dom = (function () {
    ...
    let showPhotoPosts = async function (skip, top, filterConfig) {
        document.body.getElementsByTagName('section')[0].textContent = '';
        let array = await model.getPhotoPosts(skip, top, filterConfig);
    ...
    let removePhotoPost = async function (id, skip, top, filterConfig) {
        let flag = await model.deletePhotoPost(id);
        if (flag === 'true') {
    ...
    async function setAuthorInit() {
        let authorSet = new Set();
        let size = await model.getPhotoPostArraySize();
        let posts = await model.getPhotoPosts(0, size);
        for (let i = 0; i < posts.length; i++) {
            authorSet.add(posts[i].author);
        }

        let options = '';
        authorSet.forEach(function (item) {
            options += `<option value="${item}">\n`;
        });
        document.getElementById('author-suggestions').innerHTML = options;
    }

    async function setHashTagInit() {
        ...
        let options = '';
        hashTagsSet.forEach(function (item) {
            options += `<option value=${item}>\n`;
        });
        document.getElementById('tag-suggestions').innerHTML = options;
    }

    let likePost = async function (id) {
        let post = document.getElementById(id);
        let like = post.getElementsByClassName('like')[0];
        let countOfLikes = post.getElementsByClassName('numberOfLikes')[0];
        console.log((await model.getPhotoPost(id)));
        let postResult = await model.getPhotoPost(id);
        countOfLikes.textContent = postResult.likes.length;
        like.setAttribute('src', 'img/like.png');
    };

    let unlikePost = async function (id) {
        let post = document.getElementById(id);
        let like = post.getElementsByClassName('like')[0];
        let countOfLikes = post.getElementsByClassName('numberOfLikes')[0];
        console.log((await model.getPhotoPost(id)));
    ...
    let drawEdit = function (post = null) {
        ...
        if (post) imgDropArea.setAttribute('src', post.photoLink);

        let incorrectPhoto = document.createElement('div');
        incorrectPhoto.className = 'incorrect-inform';
        events.dragAndDrop(dropArea, incorrectPhoto, imgDropArea);

        iconDown.appendChild(dropArea);
        iconDown.appendChild(imgDropArea);
        iconDown.appendChild(incorrectPhoto);
        ...
        let incorInf = document.createElement('div');
        incorInf.className = 'incorrect-inform';

        let editDescr = document.createElement('div');
        editDescr.className = 'edit-descr';

        let tArDescr = document.createElement('textarea');
        tArDescr.className = 'description-edit';
        tArDescr.id = 'descr';
        tArDescr.setAttribute('placeholder', 'Discription');

        editDescr.appendChild(tArDescr);
        section.appendChild(editDescr);

        let editHash = document.createElement('div');
        editHash.className = 'edit-hash';
        let tArHash = document.createElement('textarea');
        tArHash.className = 'description-edit';
        tArHash.id = 'hash';
        tArHash.setAttribute('placeholder', 'Hashtags');
        if (post) {
            tArDescr.textContent = post.description;
            tArHash.textContent = post.hashtags.join(' ');
        }

        editHash.appendChild(tArHash);
        section.appendChild(editHash);


        let bDone = document.createElement('div');
        bDone.className = 'button-done';
        bDone.innerHTML = 'Done';

        section.appendChild(incorInf);
        section.appendChild(bDone);

        events.clickDoneButton(post, newP, imgDropArea, incorInf, date, bDone, tArHash, tArDescr);
    };

    function drawWindow(idText) {
        document.getElementById(idText).showModal();
    }
