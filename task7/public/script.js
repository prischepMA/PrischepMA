let photoPosts = [
    {
        ...
        likes: ['Бутырчик Андрей', 'Валай Александр'],
        removed: false
    },
    {
        id: '2',
...
let modul = (function () {
        ...
        if (filterConfig) {
            ...
            if (filterConfig.author) {
                posts = posts.filter(function (item) {
                    return item.author === filterConfig.author && item.removed !== true;
                });
            }

            if (filterConfig.createdAt) {
                posts = posts.filter(function (item) {
                    return Date.parse(item.createdAt) > Date.parse(filterConfig.createdAt) && !item.removed;
                });
            }
            ...
            if (filterConfig.hashtags) {
                posts = posts.filter(function (postItem) {
                    ...
                    return filterConfig.hashtags.every(function (item) {
                        return postItem.hashtags.includes(item) && !item.removed;
                    })
                })
            }
        }
    ...
    let editLike = function (authorName, id) {
        let index = photoPosts[id].likes.findIndex(item => item === authorName);

        if (index === -1) {
            photoPosts[id].likes.push(authorName);
        }
        else {
            photoPosts.splice(index, 1);
        }
...
modul.editLike('Прищеп Максим', 1);
